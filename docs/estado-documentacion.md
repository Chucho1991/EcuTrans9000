...
- README: OK.
  ...
  - README actualizado para reflejar `Configuración de accesos`, `Clientes`, `Vehículos`, `Bitácora`, `Descuentos de viajes` y `Consulta por placas`, incluyendo permisos dinámicos por rol, reglas de importación, almacenamiento binario, tabla de equivalencia por destino, descuentos por chofer y exportación financiera por placa.
- Agent/OpenAPI: OK.
  - `agent.md` refleja el estado actual de `CLIENTES`, `VEHICULOS`, `BITACORA`, `DESCUENTOS DE VIAJES` y `CONSULTA POR PLACAS`.
...
- README documenta:
  ...
  - módulo `Descuentos de viajes` con CRUD, importación Excel por placa, estados y borrado lógico restringido a `SUPERADMINISTRADOR`,
